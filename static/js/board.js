...
if (scrollParam != null) scrollParam = parseInt(scrollParam);
if (refreshParam != null) refreshParam = parseInt(refreshParam);
else refreshParam = 5;
function scrollPage() {
  ...
  $("html, body").animate(
    ...
    function () {
      $("html, body").animate(
        ...
        len * 20,
        function () {
          setTimeout(refresh, refreshParam * 1000);
        }
      );
    }
  );
...
function genCfGhostData() {
  var result = `@contest "${
    document.getElementById("contest_title").innerHTML
  }"\n`;
  result += `@contlen ${
    parseInt(document.getElementById("time_elapsed").getAttribute("sec")) / 60
  }\n`;
      ...
      if (prob.hasAttribute("rec")) {
        ...
        for (var k = 0; k < rec.length; k++) {
          if (rec.charAt(k) == "A" || rec.charAt(k) == "R") {
            cnt++;
            submissions.push(
              `@s ${i + 1},${probId},${cnt},${time},${
                rec.charAt(k) == "A" ? "OK" : "RJ"
              }`
            );
            time = 0;
          } else {
