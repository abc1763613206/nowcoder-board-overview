...
function renderBoard(){
    ...
    $('#contest_time').text("Contest Time: " + secondsToDateTime(raw_data.contest.start_time) + " - " + secondsToDateTime(raw_data.contest.end_time));
    $('#sponsors').text("Brought to you By " + raw_data.contest.organization + ".");
    ...
    timeElapsed = parseInt(new Date().getTime() / 1000) - raw_data.contest.start_time;
    $('#time_elapsed').attr('sec', timeElapsed);
    if (curTime >= raw_data.contest.start_time && curTime <= raw_data.contest.end_time) {
    ...
    for (var i = 0; i < sorted.length; i++) {
      ...
      var node = (
        '<tr id="t' + team + '" class="type1">' +
        '<td class="realrank">'+tm.extra["real_rank"]+'</td>' +
        '<td class="rank"></td>' +
        '<td class="schoolrank"></td>' +
        '<td class="school">' + tm.organization + '</td>' +
        '<td class="team" members="' + tm.team_id + '">' +
        tm.name + '</td>' +
